...
Round~1 loads the cache built from Round~0 evasive vectors and raises
$\tau$ to $0.65$, achieving DR~$= 91.37\%$.
Round~2 exhibits a minor dip (DR~$= 89.87\%$, $\tau=0.67$) as the attacker
retrains and discovers an adjacent off-cluster evasion region not yet covered;
the updated cache closes that gap and Round~3 ($\tau=0.69$) recovers to
DR~$= 92.43\%$.
