...
learning attacker can evade \emph{both} components simultaneously by
framing dual-paradigm evasion as a single reward-maximisation problem.
After 200 training episodes, our Parameterized\-DQN attacker finds an
action region invisible to Suricata and an Iso\-la\-tion\-Forest
anomaly detector, achieving 0\% detection across five independent seeds.
...
signals. We use $\lambda = 0.5$ as the default; the sensitivity study
...
Default penalty $\lambda$ & 0.5 \\
Default threshold $\tau$ & 0.63 \\
\bottomrule
\end{tabular}
\end{table}
...
(\texttt{jasonish/\allowbreak suricata:7.0.5}) subscribed to the ET~Open rule set.
...
fully reproducible from a single seed via the \texttt{seed\_every\-thing()}
...
\midrule
C-01 & Simulation & Rnd  & IF ($\tau$=0.63)  & 200         & 0.5 & No \\
C-02 & Simulation & FS   & IF ($\tau$=0.63)  & 200         & 0.5 & No \\
C-03 & Simulation & LU   & IF ($\tau$=0.63)  & 200         & 0.5 & No \\
C-04 & Simulation & pDQN & IF ($\tau$=0.63)  & 200         & 0.5 & No \\
C-05 & Simulation & pDQN & IF ($\tau$=0.63)  & 200         & 0.5 & Yes \\
C-06 & Simulation & pDQN & AE ($\tau$=0.63)  & 200         & 0.5 & No \\
C-07 & Simulation & pDQN & AE ($\tau$=0.63)  & 200         & 0.5 & Yes \\
C-08 & Simulation & pDQN & IF (5 seeds)      & $5\times200$& 0.5 & No \\
C-09 & Simulation & pDQN & IF (4 rounds)     & $4\times200$& 0.5 & Per round \\
C-10 & Simulation & pDQN & IF ($5\times5$)   & $25\times200$& See grid & No \\
C-11 & Lab        & pDQN & Suricata\,+\,IF   & 200         & 0.5 & No \\
C-12 & Lab        & pDQN & Suricata\,+\,IF   & 200         & 0.5 & Yes \\
\bottomrule
\end{tabular}
\end{table*}
...
identical conditions ($\tau=0.63$, $\lambda=0.5$). The ParameterizedDQN
(C-04) is the \emph{only} attacker that successfully evades both Suricata
and the IsolationForest detector, achieving DR~$= 0.00$ with 15 double
blind spots. All three simpler baselines are detected in every episode
(DR~$= 1.00$), confirming that N3 reward shaping is necessary for
ML-evasion at this threshold. LinUCB achieves the same result as the
random baseline (DR~$= 1.00$, DBS~$= 0$), confirming that contextual-bandit
exploration without an ML-penalty gradient cannot locate the low-anomaly
parameter region. FixedScript's notably high CAE (65.72) reflects its
fixed repertoire of medium-intensity actions whose anomaly scores are
consistently high. Notably, pDQN's CAE (9.21) is \emph{lower} than
Random/LinUCB (9.82), showing that N3 shaping actively steers the
attacker toward the lowest-anomaly region of the feature space.

\begin{table*}[t]
\caption{Attacker baseline comparison (200 episodes, simulation, IF $\tau=0.63$, $\lambda=0.5$).
...
\midrule
RandomAttacker      & C-01 & 1.00 &  9.82 &  0 \\
FixedScriptAttacker & C-02 & 1.00 & 65.72 &  0 \\
LinUCBAttacker      & C-03 & 1.00 &  9.82 &  0 \\
ParameterizedDQN    & C-04 & \textbf{0.00} & \textbf{9.21} & \textbf{15} \\
\bottomrule
\end{tabular}
\end{table*}
...
in every episode. For the N3 ablation we set $\lambda=0$ while
keeping the IsolationForest active (preventing the
NullDefence shortcut), so the attacker optimises for Suricata evasion
only and receives no ML-penalty gradient. DR rises to 1.00
(CAE~$= 9.79$): the ML detector catches every episode, confirming that
N3 reward shaping is the mechanism that steers the attacker into the
low-anomaly evasive cluster.
Each mechanism is individually necessary; no single mechanism is sufficient.

\begin{table}[t]
\caption{Ablation study (200 episodes, simulation, IF $\tau=0.63$, seed~0).
``Full'' = N1+N3 both active. N2 (auto-remediation) is a post-run defender
mechanism and does not affect attacker training.
N3 ablation keeps IF active ($\lambda=0$) to isolate the reward-shaping effect.}
\label{tab:ablation}
\centering
\small
\begin{tabular}{l r r}
...
Without N3 ($\lambda=0$, IF active) & 1.00 & 9.79 \\
\bottomrule
\end{tabular}
\end{table}
...
An additional single-seed run at $\tau=0.61$ (between the two heatmap rows)
yields DR~$= 1.00$, confirming that the transition is abrupt: no gradual
evasion region exists in the $[0.60, 0.63]$ interval.
...
the prior round \emph{and} raises the detection threshold by
$\Delta\tau = 0.02$ (from $\tau=0.63$ in Round~0 to $\tau=0.69$ by
Round~3); the attacker then retrains from scratch against the updated
defence. Round~0 (no cache, $\tau=0.63$) serves as the baseline
(DR~$= 0.00$). Round~1 loads the initial cache and raises $\tau$ to
$0.65$, immediately raising DR to 91.37\%. A minor
\emph{non-monotonic dip} appears in Round~2 ($\tau=0.67$, DR falls
to 89.87\%) because the attacker retrains and finds an adjacent
off-cluster evasion region not yet represented in the cache; by
Round~3 ($\tau=0.69$) the updated cache closes that gap and DR
recovers to 92.43\%. The system thus converges near 92\% DR within
four rounds.

\begin{table*}[t]
\caption{Arms race dynamics: 4 rounds of attacker--defender co-evolution (C-09).
Each round the defender loads the N2 cache from the previous round and raises
$\tau$ by $\Delta\tau=0.02$; the attacker retrains. Non-monotonic convergence
(Round~2 dip) is discussed in Section~\ref{sec:disc:n2}.}
\label{tab:armsrace}
\centering
\small
\begin{tabular}{r r r r r}
\toprule
Round & $\tau$ & DR & CAE & $\Delta$DR \\
\midrule
0 (no cache)  & 0.63 & 0.0000 &  9.2119 & --- \\
1             & 0.65 & 0.9137 & 11.8313 & $+91.4$\,pp \\
2             & 0.67 & 0.8987 & 11.7774 & $-1.5$\,pp \\
3             & 0.69 & 0.9243 & 11.8361 & $+2.6$\,pp \\
\bottomrule
\end{tabular}
\end{table*}
...
\centering
\small
\begin{tabular}{L{1.5cm} L{1.5cm} r r r}
\toprule
Source & Target & Bare DR & $+$N2 DR & $\Delta$DR \\
\midrule
IF sim & IF sim  & 0.0000 & 0.9137 & $+91.4$ \\
IF sim & AE sim  & 0.4250 & 0.9333 & $+50.8$ \\
IF sim & Lab (IF)& 0.1233 & 0.1027 & $-2.0$ \\
\bottomrule
\end{tabular}
\end{table}
...
The CAE (9.24) is remarkably close to the simulation value (9.21) ---
a gap of just 0.03 (0.3\%) --- providing strong quantitative evidence
that the simulation's feature encoder faithfully captures the
anomaly-score distribution of real network traffic. This close match
was not assumed but measured, and it holds despite the simulation
using synthetic normal-traffic vectors while the lab uses actual packet
captures from Docker bridge traffic.
...
anomaly_lambda: 0.5          # lambda (N3 penalty weight)
detector: if                 # "if" = IsolationForest, "ae" = Autoencoder
lab_target_ip: 172.28.0.2   # only used in --lab mode
\end{lstlisting}

\begin{lstlisting}[language=bash]
# Simulation (primary result C-04)
python src/run_experiment.py --config config.yaml

# With N2 auto-remediation cache
python src/run_experiment.py --config config.yaml \
  --evasive-cache outputs/run_primary/evasive_cache.npy
...
\texttt{lab/\allowbreak rules/\allowbreak disabled.conf}
for all lab experiments. Disabling these SIDs creates the blind spot
that allows low-intensity C2 and recon actions to pass signature
detection. The BSP of 100\% (Section~\ref{sec:lab50}) confirms that
all 0 double-blind-spot actions in C-11 correspond to actions matching
one of these disabled SIDs.
...
\small
\small
\begin{tabular}{L{2.4cm} l r r}
\toprule
Category & Intensity & DBS & Mean Anomaly \\
\midrule
Recon (\texttt{port\_scan})   & Low & 7  & 0.18 \\
C2 (\texttt{raw\_tcp\_c2})    & Low & 5  & 0.21 \\
C2 (\texttt{dns\_c2})         & Low & 3  & 0.24 \\
Exploit (\texttt{ftp\_brute}) & Low & 0  & --- \\
Exfil (\texttt{icmp\_exfil})  & Low & 0  & --- \\
All other actions             & Any & 0  & --- \\
\midrule
\textbf{Total}                &     & \textbf{15} & \textbf{0.21} \\
\bottomrule
\end{tabular}
\end{table}
...
Table~\ref{tab:cache} reports the N2 evasive-cache state across the four
arms-race rounds (C-09). Round~0 (no input cache) produces 15 evasive
feature vectors. In Rounds 1--3 the attacker retrains against the loaded
15-vector cache and raises $\tau$; since the cache covers the evasive
cluster, no new low-anomaly actions are discovered and the cache does not
grow. This confirms that the initial 15-vector cache provides comprehensive
coverage of the attacker's evasive region.

\begin{table}[H]
\caption{N2 evasive cache state across four arms-race rounds (C-09).
``Loaded'' = input cache size; ``New'' = new evasive vectors added.}
\label{tab:cache}
\centering
\small
...
Round & $\tau$ & Loaded & New \\
\midrule
0 (baseline) & 0.63 &  0 & 15 \\
1            & 0.65 & 15 &  0 \\
2            & 0.67 & 15 &  0 \\
3            & 0.69 & 15 &  0 \\
\midrule
\textbf{Total cache} & & & \textbf{15} \\
\bottomrule
\end{tabular}
\end{table}

\noindent
Round~0 establishes the baseline (DR~$= 0.00$, no cache, $\tau=0.63$).
Round~1 loads the 15-vector cache and raises $\tau$ to $0.65$,
achieving DR~$= 91.37\%$.
Round~2 ($\tau=0.67$) shows a minor dip to DR~$= 89.87\%$ as the attacker
retrains against the raised threshold; no new evasive vectors are added.
Round~3 ($\tau=0.69$) recovers to DR~$= 92.43\%$, confirming that the
15-vector cache provides stable coverage across all four rounds.
...
(Table~\ref{tab:ae_ablation}; 200 episodes, no N2 cache, seed~0,
$\tau=0.63$, $\lambda=1.0$). DR ranges from 41\% (wider hidden layer)
to 56\% (narrow bottleneck): larger-capacity autoencoders better fit
the normal-traffic distribution and therefore exhibit lower
reconstruction error on attack traffic, reducing detection sensitivity.
Despite this spread, all three are qualitatively consistent ---
DR far above IF's 0\% and well below the with-cache ceiling of 91\%
--- confirming the N2 mechanism generalises regardless of AE capacity.

\begin{table}[H]
\caption{Autoencoder architecture sensitivity (200 episodes, no N2 cache,
seed~0, $\tau=0.63$, $\lambda=1.0$). Wider networks are less sensitive
because they better model the normal-traffic distribution.}
\label{tab:ae_ablation}
\centering
\small
\begin{tabular}{L{3.2cm} r r}
\toprule
Architecture & DR & CAE \\
\midrule
$7\!\to\!4\!\to\!2\!\to\!4\!\to\!7$ (default) & 0.5603 & 9.35 \\
$7\!\to\!4\!\to\!3\!\to\!4\!\to\!7$            & 0.4900 & 8.98 \\
$7\!\to\!8\!\to\!4\!\to\!8\!\to\!7$            & 0.4127 & 8.50 \\
\bottomrule
\end{tabular}
\end{table}
