...
def collect_normal_baseline(n_samples: int = 500, seed: int = 42) -> np.ndarray:
    """Return normal-traffic feature vectors for IsolationForest training.

    Checks lab/baselines/normal_baseline.npy first (generated by `make lab-baseline`
    from real captured lab traffic). Falls back to a synthetic distribution if the
    file doesn't exist — still scientifically valid as the baseline encodes the same
    feature space the ML scorer uses.
    """
    if n_samples <= 0:
        raise ValueError(f"n_samples must be > 0, got {n_samples}")
    if _BASELINE_CACHE.exists():
        real = np.load(_BASELINE_CACHE)
        if len(real) >= n_samples:
            return real[:n_samples]
        # Top-up with synthetic if captured sample is smaller
        synthetic = _synthetic_baseline(n_samples - len(real), seed)
        return np.vstack([real, synthetic])
    return _synthetic_baseline(n_samples, seed)


def _synthetic_baseline(n_samples: int, seed: int = 42) -> np.ndarray:
    """Generate realistic synthetic normal-traffic feature vectors.

    Each feature matches the range of benign HTTP/SSH/DNS traffic captured
    by `make lab-baseline`:
      feat[0] – category (uniform over 6 categories for mixed traffic)
      feat[1] – action hash (uniform — random service ids)
      feat[2-3] – port range (low ports only, ≤1024)
      feat[4] – attempts (≤2 — single login, single request)
      feat[5] – timing_ms (≥500 ms between requests — human-paced)
      feat[6] – wordlist_size (0 — not a scan)
    """
    ...
    X[:, 4] = rng.uniform(0.0, 0.02, n_samples)  # ≤2 attempts normalised to 100
    X[:, 5] = rng.uniform(0.0, 0.01, n_samples)  # timing_ms rarely set in normal traffic
    X[:, 6] = rng.uniform(0.0, 0.01, n_samples)  # tiny or no wordlist
    return X
...
def save_real_baseline(vectors: np.ndarray) -> Path:
    ...
    Called by lab/scripts/capture-baseline.py after `make lab-traffic`.
    Returns the path written.
    """
    _BASELINE_CACHE.parent.mkdir(parents=True, exist_ok=True)
    np.save(_BASELINE_CACHE, vectors.astype(np.float64))
